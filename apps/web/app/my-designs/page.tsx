        ...
        <div className="min-h-screen bg-void relative overflow-hidden">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex items-center justify-between mb-8 animate-fade-in relative z-10">
                    ...
                    <Link href="/studio">
                        <Button className="rounded-none border-cyan bg-cyan/10 text-cyan hover:bg-cyan hover:text-void font-bold transition-all font-mono tracking-widest uppercase px-6">
                            ...
                            >
                                ...
                                <div className="aspect-square bg-gradient-to-br from-panel-highlight to-border-std flex items-center justify-center relative overflow-hidden">
                                    ...
                                    {design.status === 'completed' && design.original_image_url && (
                                        <div className="absolute inset-0 bg-void/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 z-30">
                                            <button
                                ...
                                <div className="p-4 border-t border-border-std bg-void">
                                    <p className="text-xs font-mono text-white truncate mb-2" title={design.prompt}>
            ...
            {applyDesign && selectedDesign && (
                <div className="fixed inset-0 bg-void/80 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={() => setApplyDesign(null)}>
                    <div className="bg-panel border border-border-std p-8 relative max-w-lg w-full shadow-[0_0_30px_rgba(0,240,255,0.1)] max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center justify-between mb-6 border-b border-border-std pb-4">
                            <h3 className="font-mono font-bold text-white uppercase tracking-widest">APPLY TO PRODUCT</h3>
                        ...
                        <div className="flex items-center gap-4 p-4 bg-void border border-cyan/30 mb-6 group">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-mono font-bold truncate text-white uppercase"><span className="text-cyan">{">"}</span> {selectedDesign.prompt}</p>
                        ...
                        {/* Product list */}
                        <div className="overflow-y-auto pr-2 pb-4 space-y-3 scrollbar-thin scrollbar-thumb-cyan/20 scrollbar-track-void flex-1">
                            {products?.slice(0, 12).map((product) => (
                                <Link
                                    key={product.id}
                                    href={`/gallery/${product.id}?design=${selectedDesign.id}`}
                                    className="flex items-center gap-4 p-3 bg-panel border border-border-std hover:border-cyan hover:shadow-[0_0_10px_rgba(0,240,255,0.15)] transition-all group relative overflow-hidden"
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[10px] text-text-dim font-mono tracking-widest mt-1 uppercase">
                                            {product.category} // <span className="text-magenta">₹{Number(product.base_price).toFixed(0)}</span>
                                        </p>
                                    </div>
